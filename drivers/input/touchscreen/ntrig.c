/* drivers/input/touchscreen/ntrig.c - NTRIG Touch driver
 *
							...
							char *rxbuf, size_t len)
{
	...
	if (TSLog & Dbg_L9) {
		...
		for (loop_i = 0; loop_i < 8; loop_i++) {
			pr_info("[ts-Tx]%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\\%.2x\n",
				txbuf[0+17*loop_i], txbuf[1+17*loop_i], txbuf[2+17*loop_i], txbuf[3+17*loop_i],
				txbuf[4+17*loop_i], txbuf[5+17*loop_i], txbuf[6+17*loop_i], txbuf[7+17*loop_i],
				txbuf[8+17*loop_i], txbuf[9+17*loop_i], txbuf[10+17*loop_i], txbuf[11+17*loop_i],
				txbuf[12+17*loop_i], txbuf[13+17*loop_i], txbuf[14+17*loop_i], txbuf[15+17*loop_i],
				txbuf[16+17*loop_i]
			);
		}
	}
						...
						const char *buf, size_t count)
{
	...
	ntgFW_mode[sizeof(ntgFW_mode)-1] = ntg_checksumCal(&ntgFW_mode[14], 19);
...
static void ntrig_debug_agent_fragment(struct ntg_tsData *ntg)
{
	...
	memcpy(&data_agent[14],
		&g_data_uart.data, g_data_uart.header.length - 6);
	c_sum = CalcCheck_agentsum((unsigned char *)&data_agent[8],
		g_data_uart.header.length);
	...
	if (TSLog & Dbg_L4) {
		...
		pr_info("ntrig agent [from agent] 0x%.2x, 0x%.2x, 0x%.2x, 0x%.2x, 0x%.2x, 0x%.2x, 0x%.2x, 0x%.2x\n",
						data_agent[0], data_agent[1],
						data_agent[2], data_agent[3],
						data_agent[4], data_agent[5],
						data_agent[6], data_agent[7]);

		pr_info("ntrig agent [from agent] 0x%.2x, 0x%.2x, 0x%.2x, 0x%.2x, 0x%.2x, 0x%.2x\n",
						data_agent[8], data_agent[9],
						data_agent[10], data_agent[11],
		...
		for (i = 0; i < (g_data_uart.header.length - 6 + 4)/8; i++) {
			pr_info("ntrig agent %d[from agent] %.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\n",
						...
						data_agent[14+2+i*8], data_agent[14+3+i*8],
						data_agent[14+4+i*8], data_agent[14+5+i*8],
	...
	if (TSLog & Dbg_L4) {
		...
		for (i = 0; i < ((ntg->touchEvnt->header.length - 6)/8); i++) {
			pr_info("ntrig agent [spi] %.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\n",
						g_data_uart.data[index+0+i*8],
						g_data_uart.data[index+1+i*8],
						g_data_uart.data[index+2+i*8],
						g_data_uart.data[index+3+i*8],
						g_data_uart.data[index+4+i*8],
						g_data_uart.data[index+5+i*8],
						g_data_uart.data[index+6+i*8],
						g_data_uart.data[index+7+i*8]);
		}
		for (i = 0; i < ((ntg->touchEvnt->header.length - 6)%8); i++) {
		...
		if (temp_header.flag == 0 && pre_flag == 0) {/*only one packet*/
			...
			if (pre_flag == 0 || (temp_header.flag >= 0 &&/*check flag*/
				...
				pr_info("ntrig [ts]%s: err flag 0x%x, pre_flag 0x%x, function 0x%x, pre function 0x%x\n", __func__,
					temp_header.flag, pre_flag, temp_header.function ,
					g_data_uart.header.function);
				ntrig_debug_agent_reset(ntg);
				pre_flag = 0;
				data_uart_length = 0;
...
static int ntg_parser_agent_usbcmd(struct ntg_tsData *ntg, char* usbRxbuf)
{
	...
	if (TSLog & Dbg_L4) {
		for (loop_j = 0; loop_j < 17; loop_j++) {
			pr_info("[ntrig agent form usb]%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\n",
			usbAnalyTx[0+8*loop_j], usbAnalyTx[1+8*loop_j],
			usbAnalyTx[2+8*loop_j], usbAnalyTx[3+8*loop_j],
			usbAnalyTx[4+8*loop_j], usbAnalyTx[5+8*loop_j],
			usbAnalyTx[6+8*loop_j], usbAnalyTx[7+8*loop_j]);
		}
	...
	if (TSLog & Dbg_L4)
		for (i = 0; i < NTRIG_PACKET_SIZE/8; i++) {
			pr_info("ntrig agent %d[cmd from usb] %.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\n",
						i+1, data_form_usb[0+i*8], data_form_usb[1+i*8],
						data_form_usb[2+i*8], data_form_usb[3+i*8],
						data_form_usb[4+i*8], data_form_usb[5+i*8],
...
static void compatible_tp_report(struct input_dev *idev, void *event, bool finger, bool ontip)
{
	if (!ontip) {
...
/*#else*/
static void htc_tp_report(struct input_dev *idev, struct ntg_touch_Stouch *event, bool finger, bool ontip)
{
	if (!ontip) {
		input_report_abs(idev, ABS_MT_AMPLITUDE,  0);
		input_report_abs(idev, ABS_MT_POSITION, 0x01 << 31);
	} else {
		if (finger) {
			NtrTrackedFinger *touch = (NtrTrackedFinger *)event;
			input_report_abs(idev, ABS_MT_AMPLITUDE,  0x28 << 16 | ((touch->posDx + touch->posDy)/60));
			input_report_abs(idev, ABS_MT_POSITION, 0x01 << 31 | touch->posX << 16 | touch->posY);
		} else {
			struct ntg_touch_Stouch *touch = (struct ntg_touch_Stouch *)event;
			input_report_abs(idev, ABS_MT_AMPLITUDE, ((touch->posDx << 16) | touch->bit));
			input_report_abs(idev, ABS_MT_POSITION, 0x01 << 31 | touch->posX << 16 | touch->posY);
		}
	}
}
...
int get_finger_index (int id)
{
	...
	if (first) {
		first = 0;
		memset(gFingerMap, 0xff, sizeof(int)*30);
		gFreePos = 0;
	}
	...
	if (gFreePos >= 0) {
		gFingerMap[gFreePos] = id;
		res = gFreePos;

		for (gFreePos = (gFreePos+1) % 30; gFreePos != res; gFreePos = (gFreePos+1) % 30) {
			if (gFingerMap[gFreePos] < 0) {
				break;
			}
		}
		if (gFreePos == res)
...
void ntrig_send_multi_touch_to_android(struct input_dev *idev, NtrTrackedFingersReport *multi_touch, short orien)
{
	int i = 0, index;
	...
	for (i = 0; i < fingers_num; i++) {
		...
		if (event_google_enable == 1)
			compatible_tp_report(idev, (void *)&fingers[i], true, true);
		else{
			input_report_abs(idev, ABS_MT_AMPLITUDE, 0x28 << 16 | ((fingers[i].posDx+fingers[i].posDy)/60));
			if (i < fingers_num - 1)
				input_report_abs(idev, ABS_MT_POSITION, (fingers[i].posX << 16) | (fingers[i].posY));
			else
				input_report_abs(idev, ABS_MT_POSITION, (0x01 << 31) | (fingers[i].posX << 16) | (fingers[i].posY));
		}
	}
	if (event_google_enable == 1)
		input_sync(idev);
	...
	if (((1 == fingers_num) && (fingers[0].IsRemoved)) || (0 == fingers_num)) {
		if (event_google_enable == 1) {
			...
			input_report_abs(idev, ABS_MT_AMPLITUDE, 0x00);
			input_report_abs(idev, ABS_MT_POSITION,  1<<31);
			if (TSLog & Dbg_L4)
				pr_info("[ts]Finger Leave\n");
		}
	}
...
static int ntrig_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
	   unsigned long arg)
{
...
static void touchEventReport(struct ntg_tsData *ntg)
{
	if (ntg->touchEvnt->header.type == 0x01
		...
		if (event_google_enable == 1) {
			compatible_tp_report(ntg->input_dev, &ntg->touchEvnt->stouch, true, (ntg->touchEvnt->stouch.bit & 0x02));
			input_sync(ntg->input_dev);
		} else
			htc_tp_report(ntg->input_dev, &ntg->touchEvnt->stouch, true, (ntg->touchEvnt->stouch.bit & 0x02));
		...
		if (event_google_enable == 1) {
			compatible_tp_report(ntg->input_devP, (struct ntg_touch_Stouch *)&ntg->touchEvnt->ptouch, false,
			...
			htc_tp_report(ntg->input_devP, (struct ntg_touch_Stouch *)&ntg->touchEvnt->ptouch, false,
				(ntg->touchEvnt->ptouch.bit & 0x02) || (ntg->touchEvnt->ptouch.bit & 0x4) ||
				(ntg->touchEvnt->ptouch.bit & 0x08) || (ntg->touchEvnt->ptouch.bit & 0x1));
		}
	}
...
static bool ntg_CheckSum(char *buf, struct ntg_tsData *ntg)
{
	...
	if (sum == oriSum) {
		...
		for (loop_j = 0; loop_j < (length + 6); loop_j++) {
			pr_info("[ts-Rx]%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\\%.2x\n",
						buf[0+17*loop_j], buf[1+17*loop_j], buf[2+17*loop_j], buf[3+17*loop_j],
						buf[4+17*loop_j], buf[5+17*loop_j], buf[6+17*loop_j], buf[7+17*loop_j],
						buf[8+17*loop_j], buf[9+17*loop_j], buf[10+17*loop_j], buf[11+17*loop_j],
						buf[12+17*loop_j], buf[13+17*loop_j], buf[14+17*loop_j], buf[15+17*loop_j],
						buf[16+17*loop_j]);
		}
		return false;
	}
...
{
	bool fCheckSum = true;
	int i, nIndex = 0, loop_i, contact_count, DTime = 0;
#ifdef COUNTER_DEBUG
	u32 cnt = 0, cnt_mt = 0;
#endif
	i = 0;
	if (buf[i] == 0xA5 && buf[i+1] == 0x5A &&
		...
		if (TSLog & Dbg_L1) {
			...
			for (loop_i = 0; loop_i < 8; loop_i++) {
				pr_info("[ts-Rx]%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\\%.2x\n",
					buf[0+17*loop_i], buf[1+17*loop_i], buf[2+17*loop_i], buf[3+17*loop_i],
					buf[4+17*loop_i], buf[5+17*loop_i], buf[6+17*loop_i], buf[7+17*loop_i],
					buf[8+17*loop_i], buf[9+17*loop_i], buf[10+17*loop_i], buf[11+17*loop_i],
					buf[12+17*loop_i], buf[13+17*loop_i], buf[14+17*loop_i], buf[15+17*loop_i],
					buf[16+17*loop_i]
				);
			}
		}
		ntg->touchEvnt->header.type = buf[nIndex];
		memcpy(&ntg->touchEvnt->header.length, &buf[nIndex + 1], 2);
		ntg->touchEvnt->header.flag = buf[nIndex + 3];
		ntg->touchEvnt->header.channel = buf[nIndex + 4];
		ntg->touchEvnt->header.function = buf[nIndex + 5];
			...
			if (cnt != 0x00) {
				if ((g_lastcht+1) != cnt) {
					...
					for (loop_i = 0; loop_i < 17; loop_i++)	 {
						pr_info("[ts]%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\n",
							buf[0+8*loop_i], buf[1+8*loop_i], buf[2+8*loop_i], buf[3+8*loop_i],
							buf[4+8*loop_i], buf[5+8*loop_i], buf[6+8*loop_i], buf[7+8*loop_i]);
					}
				}
				if (cnt % 100 == 0) {
		...
		if (ntg->touchEvnt->header.channel == 0x30) {
			...
			case 0x01:
				...
				break;
			case 0x02:
				ntg->touchEvnt->stouch.repID = buf[nIndex+6];
				ntg->touchEvnt->stouch.bit = buf[nIndex+7];
				memcpy(&ntg->touchEvnt->stouch.posX, &buf[nIndex + 8], 2);
				memcpy(&ntg->touchEvnt->stouch.posY, &buf[nIndex + 10], 2);
				memcpy(&ntg->touchEvnt->stouch.posDx, &buf[nIndex + 12], 2);
				memcpy(&ntg->touchEvnt->stouch.posDy, &buf[nIndex + 14], 2);
				ntg_orientate(ntg->orientate, &ntg->touchEvnt->stouch.posX, &ntg->touchEvnt->stouch.posY);
				break;
			case 0x03:
				fCheckSum = ntg_CheckSum(&buf[nIndex], ntg);
				if (fCheckSum) {
					ntg->touchEvnt->ptouch.repID = buf[nIndex + 6];
					ntg->touchEvnt->ptouch.bit = buf[nIndex + 7];
					memcpy(&ntg->touchEvnt->ptouch.posX, &buf[nIndex + 8], 2);
					memcpy(&ntg->touchEvnt->ptouch.posY, &buf[nIndex + 10], 2);
					memcpy(&ntg->touchEvnt->ptouch.tip, &buf[nIndex + 12], 2);
					memcpy(&ntg->touchEvnt->ptouch.unused, &buf[nIndex + 14], 2);
			...
			case 0x20:
				...
				case 0x61:
					if ((buf[nIndex+6] == 0x7E && buf[nIndex+11] == 0x81) ||
						...
						ntg->touchEvnt->DfuStCmd.dfuHeader.startByte = buf[nIndex+6];
						ntg->touchEvnt->DfuStCmd.dfuHeader.msgType = buf[nIndex+11];
						ntg->touchEvnt->DfuStCmd.dfuHeader.msgGroup = buf[nIndex+12];
						ntg->touchEvnt->DfuStCmd.dfuHeader.msgCode = buf[nIndex + 13];
						memcpy(&ntg->touchEvnt->DfuStCmd.dfuHeader.returnCode, &buf[nIndex+14], 4);
						if (ntg->touchEvnt->DfuStCmd.dfuHeader.msgGroup == CMDGroup) {
							...
							case CMD_CONFIG_SETTING:
								ntg->touchEvnt->DfuStCmd.dfuHeader.returnCode == 0 ?
									pr_info("[ts_Setting]OK!\n") : pr_info("[ts_Setting]Failure!\n");
								ntg->touchEvnt->DfuStCmd.dfuHeader.returnCode == 0 ?
...
static int ntg_spi_transfer(struct ntg_tsData *ntg, char *spiTxbuf)
{
	...
	int lastCnt = 0, aIdx = 0, loop_j = 0;

	mutex_lock(&ntg_D_read_lock);
	...
	while (status != FINISH) {
		...
		if (TSLog & Dbg_L2) {
			...
			for (loop_j = 0; loop_j < 8; loop_j++) {
				pr_info("[ts-Rx]%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\\%.2x %.2x %.2x %.2x\\%.2x\n",
							spiRxbuf[0+17*loop_j], spiRxbuf[1+17*loop_j], spiRxbuf[2+17*loop_j], spiRxbuf[3+17*loop_j],
							spiRxbuf[4+17*loop_j], spiRxbuf[5+17*loop_j], spiRxbuf[6+17*loop_j], spiRxbuf[7+17*loop_j],
							spiRxbuf[8+17*loop_j], spiRxbuf[9+17*loop_j], spiRxbuf[10+17*loop_j], spiRxbuf[11+17*loop_j],
							spiRxbuf[12+17*loop_j], spiRxbuf[13+17*loop_j], spiRxbuf[14+17*loop_j], spiRxbuf[15+17*loop_j],
							spiRxbuf[16+17*loop_j]
				);
			}
		}
...
static int ntrig_ts_probe(struct spi_device *dev)
{
	...
	if (board_mfg_mode() == 5) {
		pr_info("[ts] Recover mode charging!!Touch driver wouldn't probe\n");
		return 0;
	}
	...
	for (i = 0; i < 12; i++) {
		...
		if (intFlag) {
			printk(KERN_DEBUG "[ts]touch(%d) irq HIGH\n", i);
			memset(spiTxbuf, 0xAA, NTRIG_PACKET_SIZE);
			memset(spiRxbuf, 0x00, NTRIG_PACKET_SIZE);
			...
			if (spiRxbuf[8] == 0x02) {
				...
				pr_info("[ts_FwVersion]ver = %#x", ntg_fwver);
				intFlag = 0;
				break;
			}
		} else
