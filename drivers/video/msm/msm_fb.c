...
struct msmfb_info {
	...
	struct hrtimer fake_set_overlay;
	ktime_t vsync_request_time;
	unsigned fb_resumed;
	unsigned overrides;
	struct mdp_overlay fake_req;
	struct fb_info fake_info;
	int canDoFakeOVSet;
};
...
static int msmfb_overlay_set(struct fb_info *info, void __user *p)
{
	...
	/* skip overlay set during rotation */
	if (msmfb_overlay_enable == 0 && !first_overlay_set && info->node == 0) {
		PR_DISP_INFO("msmfb_overlay_enable:%d, first_overlay_set%d\n", msmfb_overlay_enable, first_overlay_set);
		msmfb->fake_req =req;
		msmfb->fake_info = *info;
		msmfb->canDoFakeOVSet = 1;
		return 0;
	}
	if (first_overlay_set > 0 && info->node == 0)
		first_overlay_set--;
	if (hrtimer_active(&msmfb->fake_set_overlay) && info->node == 0)
		hrtimer_cancel(&msmfb->fake_set_overlay);
	/**/

	PR_DISP_INFO("%s(%d) dst rect info w=%d h=%d x=%d y=%d rotator=%d mixer=%d\n", __func__, __LINE__, req.dst_rect.w, req.dst_rect.h, req.dst_rect.x, req.dst_rect.y, req.user_data[0], info->node);
...
static int msmfb_overlay_unset(struct fb_info *info, unsigned long *argp)
{
	...
	PR_DISP_INFO("%s:unset mixer %d\n", __func__, info->node);
	if(info->node == 0) {
		msmfb->canDoFakeOVSet = 0;
...
static enum hrtimer_restart msmfb_fake_set_overlay(struct hrtimer *timer)
{
	struct msmfb_info *msmfb  = container_of(timer, struct msmfb_info,
				       fake_set_overlay);

	if(msmfb->canDoFakeOVSet) {
		PR_DISP_INFO("Do %s\n", __func__);
		mdp->overlay_set(mdp, &(msmfb->fake_info), &(msmfb->fake_req));
	}
	return HRTIMER_NORESTART;
}
...
static int msmfb_overlay_play_enable(struct fb_info *info, unsigned long enable)
{
	...
	if(info->node == 0) {
		...
		if(enable) {
			...
			if (!hrtimer_active(&msmfb->fake_set_overlay)) {
				hrtimer_start(&msmfb->fake_set_overlay,
...
static int msmfb_ioctl(struct fb_info *p, unsigned int cmd, unsigned long arg)
{
	...
	case MSMFB_HISTOGRAM:
		if(!atomic_read(&mdpclk_on)) {
		    PR_DISP_WARN("MSMFB_HISTOGRAM_STOP during suspend\n");
		    ret = -EINVAL;
		} else
...
static void setup_fb_info(struct msmfb_info *msmfb, int fb_index)
{
...
static int msmfb_probe(struct platform_device *pdev)
{
	...
	if (fb->node == 0)
		memset(msmfb->fb->screen_base, 0, msmfb->fb->fix.smem_len);

	setup_fb_info(msmfb, pdev->id);
	...
	hrtimer_init(&msmfb->fake_set_overlay, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);

	msmfb->fake_set_overlay.function = msmfb_fake_set_overlay;
